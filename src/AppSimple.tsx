...
function AppSimple() {
  ...
  const generateImage = async () => {
    logger.log('🍌 GENERANDO CON NANO BANANA...');
    ...
    try {
      ...
      if (!canvasBlob) {
        logger.error('No se pudo capturar el lienzo');
        setIsGenerating(false);
        return;
      }
      
      logger.log('📸 Lienzo capturado:', canvasBlob.size, 'bytes');
      ...
      logger.log('📝 Prompt completo:', fullPrompt);
      ...
      logger.log('🎨 Generando imagen con URL:', imageUrl);
      ...
      if (canvasBlob.size > 1000) { // Solo si hay contenido significativo en el lienzo
        ...
        reader.onloadend = async () => {
          ...
          logger.sensitive('Using Gemini API', API_KEY);
          ...
          try {
            ...
            if (response.ok) {
              ...
              if (analysis) {
                logger.log('🔍 Análisis del lienzo:', analysis);
                // Podríamos usar este análisis para mejorar el prompt en futuras versiones
              }
            }
          } catch (error) {
            logger.log('⚠️ No se pudo analizar el lienzo con Gemini:', error);
          }
        };
